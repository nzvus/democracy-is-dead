...
interface VotingFactorSectionProps {
  factor: Factor; isActive: boolean; candidates: Candidate[]; votes: Record<string, Record<string, number>>; maxScale: number; step: number; isHost: boolean;
  onToggle: () => void; onVote: (candId: string, val: number) => void; onStaticUpdate: (candId: string, val: number) => void; onShowInfo: (candidate: Candidate) => void;
}

export default function VotingFactorSection({ factor, isActive, candidates, votes, maxScale, step, isHost, onToggle, onVote, onStaticUpdate, onShowInfo }: VotingFactorSectionProps) {
  const { t } = useLanguage()
                            ...
                            <div className="flex justify-between items-end mb-4">
                                <div className="flex items-center gap-3">
                                    <CandidateTooltip candidate={candidate}>
                                        <div className="w-10 h-10 rounded-lg bg-gray-800 overflow-hidden shrink-0 border border-gray-700 relative group-hover:ring-2 ring-indigo-500 cursor-help">
                                    ...
                                    <span className="font-bold text-lg leading-none border-b border-transparent hover:border-gray-500 transition-colors cursor-pointer" onClick={() => onShowInfo(candidate)}>{candidate.name}</span>
                                </div>
                                {!isStatic && <div className={`w-12 h-10 flex items-center justify-center rounded-lg text-lg font-bold font-mono transition-colors text-white ${barColor}`}>{score}</div>}
                            </div>
                            {isStatic ? (
                                <div className="bg-black/20 p-3 rounded-xl border border-gray-800 flex items-center justify-between">
                                    ...
                                    {isHost ? <input type="number" value={staticVal} onChange={(e) => onStaticUpdate(candidate.id, Number(e.target.value))} className="w-24 bg-gray-900 border border-gray-700 rounded-lg px-2 py-1 text-right font-mono font-bold focus:border-indigo-500 outline-none" /> : <span className="font-mono font-bold text-lg">{staticVal}</span>}
                                </div>
                            ) : (
                                <div className="relative">
                                    <input type="range" min="0" max={maxScale} step={step} value={score} onChange={(e) => onVote(candidate.id, Number(e.target.value))} className={`w-full h-10 bg-gray-800 rounded-xl appearance-none cursor-pointer touch-none shadow-inner accent-${isLowerBetter ? 'amber' : 'indigo'}-500`} />
                                    <div className="flex justify-between px-1 mt-1 text-[9px] text-gray-600 font-bold uppercase tracking-widest"><span>0</span><span>{maxScale}</span></div>
                                </div>
                            )}
                        </div>
                    )
