...
export const ChatWidget = ({ lobbyId, userId }: { lobbyId: string, userId: string }) => {
  ...
  // NOTE: nickname should come from a context in a real app, simplified here
    ...
    <>
      ...
      {!isOpen && (
        <button 
          ...
          className="fixed bottom-6 right-6 z-50 group flex items-center gap-3 pl-5 pr-2 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-full shadow-[0_0_25px_rgba(79,70,229,0.5)] transition-all duration-300 hover:scale-105 border border-indigo-400/50"
        >
      ...
      >
        ...
        <div className="p-4 bg-gray-900/80 border-b border-white/10 flex justify-between items-center backdrop-blur-md">
          <div>
            ...
            <span className="text-[10px] text-green-400 font-mono tracking-wider ml-6 block">
              {/* [FIX] Use unicode to avoid literal string error */}
        ...
        {/* Messages Area */}
        <div className="flex-1 overflow-y-auto p-4 bg-gradient-to-b from-[#0b0f19] to-[#111827] scrollbar-thin scrollbar-thumb-gray-800">
          {messages.length === 0 && (
          ...
          <div className="flex flex-col gap-1"> {/* Tighter spacing */}
            {messages.map((m, index) => {
                const prevMsg = messages[index - 1];
                // Group if same user and less than 2 mins apart
                ...
                <MessageBubble 
                    ...
                    showAvatar={!isSequence} // Only show avatar if NOT a sequence
                />
                );
            })}
          </div>
          ...
          {typingUsers.length > 0 && (
            <div className="text-[10px] text-indigo-400 italic ml-12 mt-2 animate-pulse">
